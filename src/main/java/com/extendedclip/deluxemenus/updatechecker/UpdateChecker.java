...
public class UpdateChecker extends Listener {
  ...
  public UpdateChecker(final @NotNull DeluxeMenus instance) {
    ...
    instance.getScheduler().runGlobalAsync(() -> {
      if (check()) {
        // Listener registration is global on both Paper and Folia, so the global scheduler is
        // safe here.
        instance.getScheduler().runGlobal(() -> register());
      }
    });
  }
